...
export const placeOrder = async (request, response, next) => {
    try {
        ...
        const customerinfo = await Customer.findOne({_id:request.body._id})
        ...
        if (!customerinfo)
            ...
            const billAmount = await Promise.all(
                orderIdArray.map(async (id) => {
                    const item = await OrderItems.findById(id).populate("product")
...
export const getOrderDetails = async (request, response, next) => {
    try {
        ...
        const customer = await Customer.findById({ _id: request.body.id })
        ...
        if (!customer)
            ...
            if (order.length == 0)
                ...
                return response.status(200).json({ order:order,ordercount:ordercount, status: true })
            }
        }
    }
